//! @title Generic Fixed-Point FIR Filter
//! @file fir.sv
//! @author Damian Lugano
//! @date 19-5-2026
//!
//! - Generic signed fixed-point FIR filter with `N_TAPS` coefficients. 
...
module fir #(
    ...
    localparam int NB_ACC   = NB_O + $clog2(N_TAPS);
    localparam int NBF_ACC  = NBF_TAPS;
    ...
    logic signed [NB_O    - 1 : 0] prod_ts_w [N_TAPS - 1 : 0];
    ...
    //! Taps product
    generate
        for (i = 0; i < N_TAPS; i++) begin : gen_product
            assign prod_next[i] = i_taps[i] * data_reg[i];
        end
    endgenerate
    ...
    generate
        for (i = 0; i < N_TAPS; i++) begin : gen_truncNsat_prod
            truncNsat #(
                .NB_I   (NB_PROD),
                .NBF_I  (NBF_PROD),
                .NB_O   (NB_O),
