...
module regmap # ( 
    ...
    always_ff @(posedge i_clk or negedge i_rst_n) begin
        if(!i_rst_n) begin
            ...
            if (du_read_pending_r) begin
                ...
                if(i_is_write) begin  // Write registers
                    ack_r <= 1'b1;

                    case (i_addr)
                        ADDR_PRBS_CTRL: begin
                            prbs_start_r     <= i_wdata[0];
                            prbs_stop_r      <= i_wdata[1];
                            prbs_order_sel_r <= i_wdata[4 : 2];
                        end

                        ADDR_PRBS_SEED: begin
                            prbs_seed_r <= i_wdata[14 : 0];
                        end

                        ADDR_FIR_TAP_0: begin
                            fir_taps_r[0] <= $signed(i_wdata[NB_COEFF - 1 : 0]);
                        end

                        ADDR_FIR_TAP_1: begin
                            fir_taps_r[1] <= $signed(i_wdata[NB_COEFF - 1 : 0]);
                        end

                        ADDR_FIR_TAP_2: begin
                            fir_taps_r[2] <= $signed(i_wdata[NB_COEFF - 1 : 0]);
                        end

                        ADDR_FIR_TAP_3: begin
                            fir_taps_r[3] <= $signed(i_wdata[NB_COEFF - 1 : 0]);
                        end

                        ADDR_FIR_TAP_4: begin
                            fir_taps_r[4] <= $signed(i_wdata[NB_COEFF - 1 : 0]);
                        end

                        ADDR_FIR_TAP_5: begin
                            fir_taps_r[5] <= $signed(i_wdata[NB_COEFF - 1 : 0]);
                        end

                        ADDR_FIR_TAP_6: begin
                            fir_taps_r[6] <= $signed(i_wdata[NB_COEFF - 1 : 0]);
                        end

                        ADDR_FIR_TAP_7: begin
                            fir_taps_r[7] <= $signed(i_wdata[NB_COEFF - 1 : 0]);
                        end

                        ADDR_FIR_TAP_8: begin
                            fir_taps_r[8] <= $signed(i_wdata[NB_COEFF - 1 : 0]);
                        end

                        ADDR_FIR_TAP_9: begin
                            fir_taps_r[9] <= $signed(i_wdata[NB_COEFF - 1 : 0]);
                        end

                        ADDR_FIR_CTRL: begin
                            fir_commit_r <= i_wdata[0];
                            fir_enable_r <= i_wdata[1];
                        end
                        
                        ADDR_DU_CTRL: begin
                            du_arm_r   <= i_wdata[0];
                            du_mode_r  <= i_wdata[2:1];
                        end

                        ADDR_DU_THRESHOLD: begin
                            du_threshold_r <= i_wdata[NB_SAMPLE - 1 : 0];
                        end

                        ADDR_DU_RDADDR: begin
                            du_rdaddr_r <= i_wdata[DU_W_ADDR - 1 : 0];
                        end

                        ADDR_DU_SELECT: begin
                            du_select_r <= i_wdata[1 : 0];
                        end

                        ADDR_CLKMEAS_CTRL: begin
                            clkmeas_start_r <= i_wdata[0];
                        end

                        ADDR_CLKMEAS_WINDOW: begin
                            clkmeas_window_r <= i_wdata[WIN_W - 1 : 0];
                        end
                    ...
                    endcase
                end else if (i_addr == ADDR_DU_RDDATA) begin
                    if (&i_du_status) begin
    ...
    assign o_fir_enable     = fir_enable_r;
    ...
    assign o_du_rd_en       = i_req && !i_is_write && (i_addr == ADDR_DU_RDDATA) && !du_read_pending_r && (&i_du_status);
